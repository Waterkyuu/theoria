const zhCN = {
	translation: {
		benchmark: {
			automaticChecks: "自动校验",
			filterAuthor: "按作者筛选",
			checkKinds: {
				answer: "最终答案",
				file_exists: "文件存在",
				file_text: "文件文本",
				file_json: "JSON 输出",
				python: "Python 验证器",
			},
			title: "Benchmarks",
			description: "选择一套评测题，挂载到工作区，对比你的 Agent。",
			add: "新建",
			all: "全部 Benchmark",
			tags: "标签",
			searchTags: "搜索标签…",
			sortBy: "排序方式",
			author: "作者",
			allAuthors: "全部作者",
			search: "搜索 Benchmark",
			mount: "挂载",
			mountTitle: "挂载 {{name}}",
			mounted: "Benchmark 已挂载",
			workspace: "工作区",
			chooseWorkspace: "选择工作区",
			noWorkspaces: "请先在侧边栏创建工作区。",
			caseCount: "{{count}} 个 Case",
			empty: "没有匹配的 Benchmark。可以新建或清除筛选条件。",
			loadFailed: "无法加载 Benchmark，请重试。",
			retry: "重试",
			more: "加载更多",
			drafts: "草稿",
			newTitle: "新建 Benchmark",
			import: {
				action: "导入文件夹",
				title: "导入 Benchmark 文件夹",
				chooseFolder: "选择文件夹",
				chooseFolderTitle: "选择 Benchmark 模板文件夹",
				noFolder: "尚未选择文件夹",
				folderHelp: "请选择包含 benchmark.json 的文件夹。",
				previewFailed: "此文件夹不是可识别的 Theoria 模板。",
				summary: "{{cases}} 个 Case · {{files}} 个文件",
				issues: "可在草稿中修复的问题",
				confirm: "导入草稿",
				imported: "Benchmark 草稿已导入",
			},
			name: "名称",
			descriptionLabel: "描述",
			tag: "标签",
			chooseTag: "选择标签",
			newTag: "创建标签",
			tagName: "标签名称",
			icon: "图标",
			tagCreated: "标签已创建",
			createTag: "创建标签",
			cases: "Cases",
			caseName: "Case 名称",
			prompt: "任务要求",
			expected: "预期答案",
			timeout: "限时（分钟）",
			addCase: "添加 Case",
			removeCase: "删除 Case",
			removeCaseConfirm: "从草稿中删除此 Case？",
			save: "保存草稿",
			publish: "发布",
			saved: "草稿已保存",
			published: "Benchmark 已发布",
			back: "返回 Benchmark",
			draftUntitled: "未命名草稿",
			noDrafts: "还没有保存的草稿。",
			version: "版本 {{number}}",
			requirements: "任务要求",
			checks: "验证规则",
			files: "起始文件",
			noFiles: "无起始文件",
			file: {
				add: "添加输入文件",
				replace: "替换",
				preview: "预览",
				save: "保存文件",
				size: "{{count}} 字节",
				truncated: "预览已截断",
				binary: "暂不支持二进制内容预览。",
				contents: "文件内容",
				chooseTitle: "选择 Benchmark 文件",
				chooseApplication: "选择用于打开此文档的应用",
				outputPath: "输出路径",
				expectedContent: "预期内容",
				checkKind: "验证类型",
				chooseCheck: "选择验证方式",
				addCheck: "添加验证",
				noValidator: "尚未上传验证器",
				uploadValidator: "上传验证器",
			},
			duplicate: "创建副本",
			edit: "编辑",
			archive: "归档",
			archiveTitle: "归档 Benchmark？",
			archiveDescription:
				"归档后将不再出现在默认目录，也不能挂载到新的工作区；已有挂载和结果仍可访问。",
			archiveConfirm: "归档 Benchmark",
			archived: "Benchmark 已归档",
			copied: "已创建可编辑副本",
			unmount: "取消挂载",
			unmountConfirm: "取消挂载此 Benchmark？已有评测结果会保留。",
			unmounted: "已取消挂载",
			updateMount: "更新到版本 {{number}}",
			updateMountTitle: "更新挂载版本？",
			updateMountDescription:
				"后续运行将从版本 {{current}} 更新到版本 {{latest}}，已有 Task 结果不会改变。",
			updateMountConfirm: "更新挂载",
			mountUpdated: "挂载版本已更新",
			configure: "配置评测",
			agents: "本地 Agent",
			modelHint: "使用各本地 Agent 产品自身的模型配置。",
			fileAccess: "文件访问",
			commandExecution: "命令执行",
			read_only: "只读文件",
			allow_edits: "允许编辑文件",
			deny: "禁止执行命令",
			ask: "执行前询问",
			allow: "允许执行命令",
			preflight: "检查运行条件",
			checked: "运行条件检查完成",
			start: "开始评测",
			started: "评测已启动",
			cancelRun: "取消运行",
			cancelling: "正在取消",
			cancelRequested: "已请求取消评测",
			rerun: "重新运行",
			rerunDescription:
				"发布版本与完整 Case 集保持不变，可以重新选择 Agent 和权限。",
			startRerun: "开始重新运行",
			rerunStarted: "重新运行已启动",
			restoreMountTitle: "恢复历史挂载？",
			restoreMountDescription:
				"此 Benchmark 已不在工作区中。是否先恢复当时的精确版本，再重新运行？",
			restoreAndRerun: "恢复并重新运行",
			planned: "{{cases}} 个 Case × {{agents}} 个 Agent = {{count}} 次执行",
			checkPassed: "未发现缺失的运行条件。",
			taskStatus: {
				preparing: "准备中",
				running: "运行中",
				waiting: "等待中",
				completed: "已完成",
				failed: "失败",
				stopped: "已停止",
			},
			results: {
				progress: "进度",
				completed: "完成",
				passed: "通过",
				failed: "未通过",
				errors: "错误",
				case: "Case",
				result: "结果",
				casePassSummary: "{{passed}}/{{total}} 通过",
				caseComparison: "Case 对比",
				resize: "调整 Case 对比面板宽度",
				metric: "指标",
				status: "状态",
				totalDuration: "总耗时",
				firstToken: "首 token",
				tokens: "总 token",
				inputTokens: "输入",
				outputTokens: "输出",
				cachedTokens: "缓存",
				toolCalls: "工具调用",
				toolCall: "调用 {{count}}",
				agentDetail: "Agent 详情",
				toolDetails: "工具明细",
				tool: "工具",
				arguments: "参数",
				noToolCalls: "本次未调用工具",
				toolStatus: {
					completed: "成功",
					failed: "失败",
					incomplete: "未完成",
				},
				passRate: "通过率 {{rate}}%",
				incompleteCoverage: "结果覆盖不完整",
				metrics: "{{duration}} · {{tokens}} · {{tools}}",
				durationValue: "{{value}} 毫秒",
				tokenValue: "{{count}} 个 Token",
				toolValue: "{{count}} 次工具调用",
				noData: "无数据",
				metricCoverage: "{{value}}（{{coverage}}/{{total}} 个 Case）",
				executionDetail: "执行详情",
				requirements: "Case 要求",
				response: "Agent 回答",
				noResponse: "没有保存到回答。",
				duration: "执行耗时",
				checks: "验证结果",
				checkMessage: {
					answer_matched: "最终答案匹配。",
					answer_mismatch: "最终答案不匹配。",
					file_exists: "要求的文件存在。",
					file_missing: "缺少要求的文件。",
					text_matched: "文件文本匹配。",
					text_mismatch: "文件文本不匹配。",
					json_matched: "JSON 输出匹配。",
					json_mismatch: "JSON 输出不匹配。",
				},
				searchCases: "搜索 Case",
				filter: "筛选",
				agentFilter: "Agent",
				statusFilter: "状态",
				allAgents: "全部 Agent",
				allStatuses: "全部状态",
				noCases: "没有符合筛选条件的 Case。",
				previous: "上一页",
				next: "下一页",
				artifacts: "最终文件",
				noArtifacts: "没有记录到最终文件。",
				change: {
					added: "新增",
					modified: "已修改",
					deleted: "已删除",
					unchanged: "未修改",
				},
				state: {
					passed: "通过",
					failed: "未通过",
					queued: "排队中",
					preparing: "准备中",
					running: "运行中",
					waiting_permission: "等待权限确认",
					collecting: "收集结果中",
					evaluating: "评分中",
					stopping: "停止中",
					timed_out: "超时",
					agent_error: "Agent 错误",
					evaluation_error: "评分错误",
					interaction_required: "需要输入",
					cancelled: "已取消",
					interrupted: "已中断",
				},
			},
			structuredCase: "此 Case 包含文件或脚本验证，保留其原始内容。",
			answerHelp: "添加公开起始文件及一条或多条自动验证；所有验证均需通过。",
			invalid: "发布前请填写名称、描述、标签及每个 Case。",
			validation: {
				title: "请先修正以下字段再发布",
				unsupported_schema: "不支持此文档格式",
				invalid_name: "请填写符合长度要求的名称",
				invalid_description: "请填写符合长度要求的描述",
				tag_required: "请选择标签",
				invalid_case_count: "请添加 1 至 100 个 Case",
				duplicate_case: "Case 名称不能重复",
				invalid_prompt: "请填写完整的任务要求",
				invalid_timeout: "限时必须为 1 至 60 分钟",
				checks_required: "请至少添加一条有效验证规则",
				too_many_files: "起始文件不能超过 256 个",
				import_too_large: "导入文件夹不能超过 20 MB",
				file_too_large: "每个导入文件不能超过 5 MB",
				missing_file: "请添加或替换缺失的引用文件",
				unsafe_path: "请使用安全的相对文件路径",
				duplicate_path: "文件路径不能重复",
				invalid_expected: "请填写有效的预期值",
				invalid_json: "请填写有效 JSON",
				invalid_python: "请提供有效的 validate(workspace) Python 入口",
			},
			sort: {
				newest: "最新创建",
				updated: "最近更新",
				oldest: "最早创建",
				alphabetical: "名称排序",
			},
			authors: { platform: "平台", myself: "本人" },
			issues: {
				agent_not_installed: "Agent 未安装",
				agent_not_authenticated: "Agent 未登录",
				agent_check_failed: "无法检查 Agent 状态",
				asset_unavailable: "起始材料不可用",
				verifier_unavailable: "验证环境不可用",
			},
		},
		appName: "theoria",
		appEdition: "本地 Agent 工作台",
		mainNavigation: "主导航",
		collapseSidebar: "收起侧边栏",
		expandSidebar: "展开侧边栏",
		loadingPage: "正在加载页面",
		common: {
			done: "完成",
			cancel: "取消",
			delete: "删除",
			search: "搜索",
			clearSearch: "清除搜索",
			confirm: "确认",
		},
		errors: {
			badRequest: "请求参数有误",
			forbidden: "你没有权限执行此操作",
			invalidData: "响应数据格式错误",
			network: "无法连接到服务器",
			notFound: "请求的资源不存在",
			requestFailed: "请求失败",
			server: "服务器暂时不可用",
			timeout: "请求超时",
			tooManyRequests: "请求过于频繁，请稍后重试",
			unauthorized: "登录状态已失效，请重新登录",
		},
		navigation: {
			agentAccess: "Agent 接入",
			benchmark: "基准测试",
			newTask: "新任务",
			workspaces: "工作区",
			skills: "技能库",
			compare: "性能对比",
			comparisonHistory: "历史对比",
			runs: "运行看板",
		},
		workspaceSidebar: {
			label: "工作区侧边栏",
			workspaces: "工作区",
			addWorkspace: "添加工作区",
			newWorkspace: "新建",
			newWorkspaceTitle: "新建工作区",
			newWorkspaceDescription: "选择工作区来源并设置名称",
			workspaceNameLabel: "工作区名称",
			workspaceSourceLabel: "工作区来源",
			workspaceSource: {
				external: "导入本地",
				managed: "平台托管",
			},
			workspacePathLabel: "源文件夹",
			chooseFolder: "选择本地文件夹",
			chooseFolderTitle: "选择 Workspace 文件夹",
			createWorkspace: "创建",
			createWorkspaceSuccess: "已创建工作区“{{workspace}}”",
			createWorkspaceFailed: "创建工作区失败，请检查名称和目录路径",
			collapseWorkspaces: "收起工作区",
			expandWorkspaces: "展开工作区",
			collapseRecentTasks: "收起最近任务",
			expandRecentTasks: "展开最近任务",
			collapseWorkspace: "收起 {{workspace}}",
			expandWorkspace: "展开 {{workspace}}",
			workspaceActions: "{{workspace}} 的更多操作",
			pinWorkspace: "置顶",
			unpinWorkspace: "取消置顶",
			renameWorkspace: "重命名",
			workspaceRename: {
				title: "重命名工作区",
				description: "为“{{workspace}}”设置新的工作区名称",
				label: "工作区名称",
				save: "保存",
				success: "已重命名工作区为“{{name}}”",
			},
			workspacePin: {
				pinned: "已置顶工作区“{{workspace}}”",
				unpinned: "已取消置顶工作区“{{workspace}}”",
				failed: "工作区置顶状态更新失败，请重试",
			},
			archiveWorkspace: "归档",
			removeWorkspace: "移除",
			workspaceRemove: {
				title: "移除工作区？",
				externalDescription:
					"“{{workspace}}”的任务和技能挂载会被删除，本地原文件夹不会被修改或删除",
				managedDescription:
					"“{{workspace}}”的任务、技能挂载和平台托管模板目录都会被永久删除。请输入工作区名称确认",
				confirmationLabel: "输入 {{workspace}} 以确认",
				confirm: "移除工作区",
				success: "已移除工作区“{{workspace}}”",
				failed: "工作区移除失败，请重试",
			},
			conversationsLabel: "任务",
			deleteConversation: "删除",
			pinTask: "置顶",
			unpinTask: "取消置顶",
			taskRename: {
				title: "重命名任务",
				description: "为“{{task}}”设置新的任务名称",
				label: "任务名称",
				save: "保存",
				success: "已重命名任务为“{{title}}”",
			},
			taskDelete: {
				title: "删除任务？",
				description:
					"“{{task}}”及其隔离运行目录和结果文件会被永久删除，此操作无法撤销",
				confirm: "删除任务",
				success: "已删除任务“{{task}}”",
				failed: "任务删除失败，请重试",
			},
			taskPin: {
				pinned: "已置顶任务“{{task}}”",
				unpinned: "已取消置顶任务“{{task}}”",
				failed: "任务置顶状态更新失败，请重试",
			},
			mockConversation: "当前任务",
			mockConversationActions: "{{conversation}}的更多操作",
			newConversation: "新建任务",
			pinned: "已置顶",
			recent: "最近",
			renameConversation: "重命名",
			benchmarks: "基准测试",
			benchmarkMountActions: "{{benchmark}}的更多操作",
			pinBenchmarkMount: "置顶",
			unpinBenchmarkMount: "取消置顶",
			benchmarkMountPin: {
				pinned: "已置顶基准测试“{{benchmark}}”",
				unpinned: "已取消置顶基准测试“{{benchmark}}”",
				failed: "基准测试置顶状态更新失败，请重试",
			},
			benchmarkUnmount: {
				title: "取消挂载基准测试？",
				description: "“{{benchmark}}”将从当前工作区移除，已有评测结果会保留。",
				confirm: "取消挂载",
				success: "已取消挂载基准测试“{{benchmark}}”",
				failed: "取消挂载基准测试失败，请重试",
			},
			mountedSkills: "已挂载技能",
			mountedSkillActions: "{{skill}}的更多操作",
			unmountSkillFromWorkspace: "移除",
			skillUnmount: {
				title: "移除挂载？",
				description: "{{skill}} 将从 {{workspace}} 的已挂载技能中移除。",
				success: "已从 {{workspace}} 移除 {{skill}} 挂载",
				failed: "移除挂载失败，请重试",
			},
			appSettings: "应用设置",
		},
		settings: {
			title: "应用设置",
			description: "管理应用的外观与语言偏好，修改会立即生效",
			theme: {
				title: "主题",
				description: "选择应用使用浅色、深色或跟随系统外观",
				menuTitle: "选择主题",
				selectorLabel: "主题，当前：{{value}}",
				options: {
					system: "跟随系统",
					light: "浅色",
					dark: "深色",
				},
			},
			language: {
				title: "语言",
				description: "选择应用界面使用的显示语言",
				menuTitle: "选择语言",
				selectorLabel: "语言，当前：{{value}}",
				options: {
					"zh-CN": "简体中文",
					"en-US": "English",
				},
				success: "语言已切换",
				failed: "语言同步失败，请重试",
			},
		},
		workspace: {
			breadcrumb: "{{workspace}} / 新任务",
			taskLabel: "任务内容",
			composerPlaceholder: "描述任务，输入 / 选择已启动的 Agent",
			composerHint: "Enter 发送，Shift + Enter 换行",
			attachFiles: "挂载文件",
			exploreMode: "探索模式",
			benchmarkMode: "基准测试模式",
			modeSelection: "选择运行模式",
			exploreDescription: "直接使用工作区当前文件",
			benchmarkDescription: "锁定输入，创建可复现的正式运行",
			benchmarkNotice: "基准测试使用独立工作区快照",
			benchmarkNoticeDetail: "每个 Agent 从相同内容开始，运行期间彼此隔离",
			selectedAgentCount: "已选择 {{count}} 个 Agent",
			runningAgents: "已启动的 Agent",
			addAgent: "添加 Agent",
			started: "已启动",
			mountedSkillCount: "{{count}} 个技能",
			skillSelection: "选择已挂载技能",
			permission: "可写入工作区",
			permissionSelection: "配置运行权限",
			filePermission: "文件访问",
			permissionReadOnly: "只读文件",
			permissionAllowEdits: "允许编辑文件",
			commandPermission: "命令执行",
			commandPermissionOption: {
				deny: "禁止执行命令",
				ask: "执行前询问",
				allow: "允许执行命令",
			},
			environment: "Agent 环境",
			viewEnvironment: "查看 Agent 环境",
			startedAgentCount: "{{count}} 个已启动",
			installed: "已安装",
			environmentDescription: "本机 Agent 的安装、启动和当前配置",
			closeEnvironment: "关闭 Agent 环境",
			sendTask: "发送任务",
			dispatched: "任务已分发到 {{count}} 个 Agent",
			runCompleted: "任务执行已完成",
			runFailed: "任务执行失败，请重试",
			workspacePath: "{{workspace}}",
			noSkills: "暂无可用技能",
		},
		taskPanel: {
			panel: "执行面板",
			loading: "正在恢复任务",
			createFailed: "任务创建失败，请检查工作区与 Agent 环境后重试",
			sections: "结果分区",
			stopAgent: "停止 {{agent}}",
			stopSuccess: "已停止 {{agent}}",
			stopFailed: "停止 {{agent}} 失败，请重试",
			toolCalls: "已调用 {{count}} 个工具",
			noAnswer: "尚未生成回答",
			fileSummary: "{{added}} 个新增，{{modified}} 个修改，{{deleted}} 个删除",
			section: {
				process: "过程",
				answer: "回答",
				files: "文件",
				metrics: "指标",
			},
			status: {
				preparing: "准备中",
				running: "运行中",
				waiting: "等待用户",
				completed: "已完成",
				failed: "失败",
				stopped: "已停止",
			},
			process: {
				preparing: "正在创建隔离工作区",
				running: "Agent 正在隔离工作区中执行",
				waiting: "Agent 需要用户输入后继续",
				completed: "执行与结果收集已完成",
				failed: "执行失败，请查看回答中的错误信息",
				stopped: "执行已由用户停止",
			},
			run: {
				you: "你",
				response: "回答",
				tokens: "tokens",
				toolCompleted: "工具调用已完成",
				toolDuration: "工具调用耗时 {{duration}}",
				activity: {
					preparing: "正在准备隔离运行目录",
					running: "正在生成回答…",
					waiting: "正在等待批准",
					completed: "执行记录已保存",
					failed: "执行失败，请检查日志",
					stopped: "执行已停止，请检查日志",
				},
				action: {
					stop: "停止",
				},
			},
		},
		taskFollowUp: {
			label: "继续任务",
			placeholder: "继续向现有 Agent 发送消息…",
			allAgents: "全部 Agent",
			agentSelection: "选择接收消息的 Agent",
			agentSelectionSummary: "{{target}}，任务中共 {{count}} 个 Agent",
			targetAgent: "仅发送给 {{agent}}",
			send: "发送继续任务",
			sent: "已发送继续任务",
			failed: "继续任务发送失败，请重试",
		},
		taskSummary: {
			title: "结果汇总",
			open: "查看结果汇总",
			close: "关闭",
			enterFullscreen: "全屏查看",
			exitFullscreen: "退出全屏",
			resize: "调整结果汇总宽度",
			readOnly: "只读 Task 级对比",
			metric: "指标",
			status: "状态",
			duration: "总耗时",
			firstToken: "首 token",
			tokens: "总 token",
			inputTokens: "输入",
			outputTokens: "输出",
			cachedTokens: "缓存",
			expandTokenDetails: "展开 Token 明细",
			collapseTokenDetails: "收起 Token 明细",
			toolCalls: "工具调用",
			toolCall: "调用 {{sequence}}",
			arguments: "参数",
			result: "结果",
			toolStatus: "状态",
			noToolCall: "无",
			expandToolCalls: "展开工具调用明细",
			collapseToolCalls: "收起工具调用明细",
			files: "文件变化",
			fileCounts: "新增 {{added}} · 修改 {{modified}} · 删除 {{deleted}}",
			unavailable: "不可用",
		},
		skills: {
			title: "技能",
			path: "~/.theoria/skills",
			description: "可复用的能力库。需要时可将技能挂载到一个或多个工作区",
			addSkill: "添加技能",
			editor: {
				editTitle: "编辑技能",
				saved: "已保存技能 {{skill}}",
				loadFailed: "无法读取技能文件，请重试。",
				retry: "重试",
				retainedFile:
					"此文件为非文本文件或超出编辑大小限制，保存时会保留原文件。",

				selectRoot: "选择根目录",
				rename: "重命名",
				delete: "删除",
				entryActions: "{{path}} 的操作",
				deleteTitle: "删除“{{path}}”？",
				deleteDescription: "文件夹中的所有文件也会删除。保存后生效。",
				noDocument: "选择或创建文件开始编辑",

				newFolder: "新建文件夹",
				folderPath: "文件夹路径",

				metadata: "元数据",
				document: "编辑区",
				directory: "目录",
				resizeDirectory: "调整目录宽度",
				moveLeft: "目录移到左侧",
				moveRight: "目录移到右侧",
				save: "保存",
				saveDisabledHint:
					"请完整填写 SKILL.md 的 name 和 description 后再保存。",
				saving: "保存中…",
				edit: "编辑",
				preview: "预览",
				newFile: "新建文件",
				filter: "筛选文件…",
				filePath: "文件路径",
				pathError: "文件路径无效或已存在，请换一个名称。",
				noFiles: "没有匹配的文件",
				saveFailed: "保存失败，请检查文件内容或重试。",
				folderHint: "保存为 {{name}}/ · 单文件上限 1 MB，总计 10 MB",
				metadataHint:
					"请填写 SKILL.md 的 name 和 description；name 为文件夹名，支持英文字母、数字、- 和 _（最多 120 字符）。",
				discardTitle: "放弃未保存的技能？",
				discardDescription: "返回后将丢失当前编辑的文件。",
				discard: "放弃更改",
			},
			addMenu: {
				simple: "简单创建",
				editor: "编辑器创建",
				title: "选择创建方式",
				platform: "在 Theoria 中创建",
				folder: "从本地文件夹导入",
				git: "从 Git 链接导入",
			},
			chooseFolderTitle: "选择技能文件夹",
			importSuccess: "已导入技能“{{skill}}”",
			importFailed: "技能导入失败，请确认文件夹中包含有效的 SKILL.md",
			updateSuccess: "已更新技能“{{skill}}”",
			updateFailed: "技能更新失败，请重试",
			loading: "正在加载技能",
			loadFailed: "无法读取技能库",
			searchPlaceholder: "按名称、来源或能力搜索",
			libraryLabel: "技能库",
			filterMenu: {
				title: "筛选技能",
				trigger: "筛选技能，当前：{{filter}}",
			},
			filters: {
				all: "全部",
				mounted: "已挂载",
				local: "本地",
				github: "GitHub",
				local_folder: "文件导入",
				platform: "平台创建",
				git: "Git",
			},
			columns: {
				skill: "技能",
				source: "来源",
				workspaces: "工作区",
				actions: "操作",
			},
			source: {
				local: "本地",
				github: "GitHub",
				local_folder: "文件导入",
				platform: "平台创建",
				git: "Git",
			},
			mountedCount: "已挂载 {{count}} 个",
			notMounted: "未挂载",
			manage: "管理",
			mount: "挂载",
			update: "更新",
			updateNamed: "更新 {{name}}",
			selectAll: "选择全部技能",
			selectNamed: "选择 {{name}}",
			deleteSelected: "删除已选技能",
			removeDialog: {
				title: "移除所选技能？",
				description:
					"将永久删除 {{count}} 个技能的 Theoria 托管副本，并解除所有工作区挂载。导入源和已有任务快照不会被修改。",
				confirm: "移除技能",
				success: "已移除 {{count}} 个技能",
				failed: "技能移除失败，请重试",
			},
			pagination: {
				label: "技能分页",
				summary: "{{start}} 至 {{end}}，共 {{total}} 条结果",
				previous: "上一页",
				next: "下一页",
			},
			gitDialog: {
				title: "从 Git 链接导入",
				description:
					"支持导入仓库根目录的单个技能，或 skills 目录下的多个技能；导入后可从远程更新",
				urlLabel: "Git 仓库链接",
				import: "导入",
				importing: "正在导入",
				success: "已从 Git 导入 {{count}} 个技能",
				failed: "Git 技能导入失败，请检查链接和仓库内容",
			},
			create: {
				title: "创建技能",
				description: "填写最小必要内容，Theoria 会生成并托管 SKILL.md",
				back: "返回技能库",
				nameLabel: "技能名称",
				nameInvalid: "技能名称只能使用英文字母、数字、空格、- 和 _",
				descriptionLabel: "描述",
				contentLabel: "主内容",
				submit: "创建技能",
				success: "已创建技能“{{skill}}”",
				failed: "技能创建失败，请检查填写内容",
			},
			mountDialog: {
				title: "管理工作区挂载",
				description: "选择以后创建任务时使用 {{skill}} 的工作区",
				loading: "正在读取工作区挂载",
				loadFailed: "无法读取 {{workspace}} 的挂载状态",
				workspacesFailed: "无法读取工作区列表",
				empty: "还没有工作区，请先创建一个工作区",
				mountedSuccess: "已挂载 {{skill}} 到 {{workspace}}",
				unmountedSuccess: "已从 {{workspace}} 移除 {{skill}} 挂载",
				updateFailed: "工作区挂载更新失败，请重试",
			},
			noResults: "没有匹配的技能",
			mock: {
				repositoryMap: {
					name: "repository-map",
					description: "映射仓库结构和关键入口",
				},
				testRunner: {
					name: "test-runner",
					description: "运行项目测试命令并提供限定范围的输出",
				},
				uiAudit: {
					name: "ui-audit",
					description: "检查桌面界面的一致性和可访问性",
				},
				benchmarkEvaluator: {
					name: "benchmark-evaluator",
					description: "使用保存的标准为基准测试输出评分",
				},
				releaseNotes: {
					name: "release-notes",
					description: "根据仓库历史生成发布说明",
				},
			},
		},
		agentNames: {
			claude: "Claude Code",
			codex: "Codex",
			opencode: "OpenCode",
			workbuddy: "WorkBuddy",
		},
		agentSelection: "选择要对比的 Agent",
		languageSelection: "切换语言",
		languages: {
			zhCN: "中文",
			enUS: "English",
		},
		tagline: "本地 Agent 性能测量",
		title: "一个任务同时运行，关键指标直接对比",
		description:
			"选择本机 Agent 产品，并行执行相同任务，横向比较响应速度、思考时间、工具调用、token 用量和最终响应",
		queryLabel: "任务内容",
		queryPlaceholder: "例如：概括这个仓库的主要功能，不要修改文件",
		selectedAgents: "已选择 {{count}} 个可用 Agent",
		compareAgents: "运行 {{count}} 个 Agent 对比",
		comparingAgents: "正在对比 {{count}} 个 Agent…",
		checkingLogin: "正在检查本地 {{agent}} 登录状态…",
		checkingProcess: "正在检查本地 {{agent}} 启动状态…",
		processCheckFailed: "无法检查本地 {{agent}} 启动状态",
		agentRunning: "已启动",
		agentReady: "未启动",
		notInstalled: "未安装",
		notLoggedIn: "未登录",
		loginCheckFailed: "无法检查本地 {{agent}} 登录状态",
		loggedIn: "{{agent}}：已登录",
		loggedInWithMethod: "{{agent}}：已通过 {{method}} 登录",
		runtimeConfiguration: "当前 {{agent}} 配置",
		currentModel: "当前模型",
		reasoningEffort: "思考强度",
		reasoningEffortLevels: {
			enabled: "默认",
			none: "无",
			minimal: "最低",
			low: "低",
			medium: "中",
			high: "高",
			xhigh: "超高",
			max: "最高",
		},
		comparisonTitle: "对比结果",
		comparisonResult: "{{agent}} 对比结果",
		agentRunRunning: "执行中…",
		agentRunFinished: "{{task}} 任务 {{agent}} 已结束",
		viewResult: "请查看结果",
		firstToken: "首 token",
		totalDuration: "总耗时",
		totalTokens: "总 token",
		inputTokens: "输入",
		outputTokens: "输出",
		reasoningTokens: "推理",
		thinkingDuration: "思考时间",
		compactionCount: "压缩次数",
		toolCallCount: "工具调用",
		toolCallsTitle: "工具明细",
		noToolCalls: "本次未调用工具",
		responseTitle: "响应",
		metricUnavailable: "不可用",
		unknownModel: "未知模型",
		requestFailed: "任务执行失败，请重试",
		comparisonHistory: {
			tagline: "本地性能记录",
			title: "历史对比",
			description:
				"回看每一次性能比较的任务、运行配置、指标、工具调用和最终响应",
			listTitle: "对比记录",
			loading: "正在加载历史记录",
			loadingDetail: "正在加载对比详情",
			loadFailed: "无法读取历史对比记录",
			detailFailed: "无法读取这条对比详情",
			emptyTitle: "还没有历史对比",
			emptyDescription: "完成一次性能对比后，结果会自动保存在这里",
			backToList: "返回对比记录",
			recordActions: "{{query}}的更多操作",
			rename: "重命名",
			renameTitle: "重命名记录",
			renameDescription: "为“{{query}}”设置新的记录名称",
			renameLabel: "记录名称",
			renameSave: "保存",
			delete: "删除",
			deleteTitle: "删除记录？",
			deleteDescription: "确认删除“{{query}}”的历史记录",
			deleteConfirm: "删除",
			loadMore: "加载更多",
			saveFailed: "结果已完成，但无法保存到历史对比",
		},
		runBoard: {
			title: "Agent 运行看板",
			searchTitle: "搜索任务",
			searchPlaceholder: "搜索任务或 Agent",
			searchEmpty: "没有找到匹配的任务",
			contextUsage: "上下文占用",
			empty: "此状态下暂无任务",
			untitledTask: "未命名任务",
			dragStatus: "拖动{{status}}面板",
			status: {
				running: "运行中",
				waiting: "等待用户",
				finish: "已完成",
				error: "异常",
			},
			statusDescription: {
				running: "正在执行",
				waiting: "需要回答后继续",
				finish: "已完成",
				error: "需要处理",
			},
		},
	},
} as const;

export { zhCN };
