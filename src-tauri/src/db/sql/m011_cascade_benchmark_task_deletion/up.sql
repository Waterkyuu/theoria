...
CREATE TABLE benchmark_tasks_new (
    ...
    rerun_of_task_id TEXT REFERENCES benchmark_tasks_new(task_id) ON DELETE SET NULL,
...
CREATE TABLE benchmark_task_agents_new (
    id TEXT PRIMARY KEY,
    task_id TEXT NOT NULL REFERENCES benchmark_tasks_new(task_id) ON DELETE CASCADE,
...
CREATE TABLE benchmark_task_cases_new (
    ...
    FOREIGN KEY (task_id, version_id) REFERENCES benchmark_tasks_new(task_id, version_id) ON DELETE CASCADE,
...
CREATE TABLE benchmark_case_executions_new (
    ...
    FOREIGN KEY (task_id, task_case_id) REFERENCES benchmark_task_cases_new(task_id, id) ON DELETE CASCADE,
    FOREIGN KEY (task_id, task_agent_id) REFERENCES benchmark_task_agents_new(task_id, id) ON DELETE CASCADE,
...
CREATE TABLE benchmark_evaluations_new (
    execution_id TEXT PRIMARY KEY REFERENCES benchmark_case_executions_new(id) ON DELETE CASCADE,
...
CREATE TRIGGER benchmark_tasks_plan_immutable BEFORE UPDATE OF task_id, version_id, request_json, idempotency_key ON benchmark_tasks
BEGIN SELECT RAISE(ABORT, 'Execution plan is immutable'); END;
CREATE TRIGGER benchmark_tasks_rerun_immutable BEFORE UPDATE OF rerun_of_task_id ON benchmark_tasks
WHEN NOT (OLD.rerun_of_task_id IS NOT NULL AND NEW.rerun_of_task_id IS NULL)
BEGIN SELECT RAISE(ABORT, 'Execution plan is immutable'); END;
